...
print("=== Logic: PPO controls Green duration; Yellow cost is INCLUDED in decision ===")

current_simulation_step = 0

# 使用 while 迴圈，因為我們在黃燈期間會手動推進 step
while current_simulation_step < TOTAL_STEPS:
    
    # 1. 讀取當前狀態 (Green Phase)
    state = get_state()
    ...
    sum_halting += (hEB + hSB)
    total_steps += 1
    if phase_idx in EB_PHASES: eb_green_steps += 1
    if phase_idx in SB_PHASES: sb_green_steps += 1

    # Log 輸出
    if current_simulation_step % 100 == 0:
        sim_t = current_simulation_step * STEP_LENGTH
        print(f"[t={sim_t:4.1f}s] qEB={qEB:3d}, qSB={qSB:3d} | hEB={hEB:3d}, hSB={hSB:3d} (Green), phase={phase_idx}")
    ...
    if action == 0:
        ...
        # 取得下一步狀態與獎勵
        new_state = get_state()
        step_reward = get_reward(new_state)
        
    else:
        # --- 切換相位 (Switch) ---
        # 這包含了：切換到黃燈 -> 跑完黃燈 -> 切換到下一個綠燈
        # 這段期間的所有 Reward 都會算在這次 Action 上
        
        # A. 切換到黃燈
        program = traci.trafficlight.getAllProgramLogics(TLS_ID)[0]
        num_phases = len(program.phases)
        yellow_phase = (phase_idx + 1) % num_phases
        
        traci.trafficlight.setPhase(TLS_ID, yellow_phase)
        traci.trafficlight.setPhaseDuration(TLS_ID, 99999)
        last_switch_step = current_simulation_step
        print(f"Step {current_simulation_step}: Green -> Yellow (Phase {yellow_phase})")

        # B. 跑完黃燈時間 (Accumulate Reward)
        yellow_reward_sum = 0.0
        
        for _ in range(YELLOW_DURATION_STEPS):
            traci.simulationStep()
            current_simulation_step += 1
            
            # 統計黃燈期間的數據
            y_state = get_state()
            yellow_reward_sum += get_reward(y_state)
            
            # 統計更新
            y_phase = y_state[12]
            phase_counts[y_phase] += 1
            yellow_steps += 1
            total_arrived_vehicles += traci.simulation.getArrivedNumber()
            
            # 佇列統計
            y_qEB = int(y_state[0] + y_state[1] + y_state[2])
            y_qSB = int(y_state[3] + y_state[4] + y_state[5])
            y_hEB = int(y_state[6] + y_state[7] + y_state[8])
            y_hSB = int(y_state[9] + y_state[10] + y_state[11])
            
            sum_qEB += y_qEB; sum_qSB += y_qSB
            sum_hEB += y_hEB; sum_hSB += y_hSB
            sum_halting += (y_hEB + y_hSB)
            total_steps += 1
            
            if current_simulation_step >= TOTAL_STEPS:
                break
        ...
        traci.trafficlight.setPhaseDuration(TLS_ID, 99999)
        last_switch_step = current_simulation_step
        print(f"Step {current_simulation_step}: Yellow -> Green (Phase {next_green_phase})")
        
        # D. 最終狀態 (Next Green Start)
        new_state = get_state()
        
        # 總獎勵 = 切換瞬間的獎勵(通常是0或很小) + 黃燈期間累積的負獎勵
        # 這樣 AI 就會知道 "Switch" 是一個昂貴的動作
        step_reward = yellow_reward_sum
    ...
    rewards_buffer.append(step_reward)
